...
### Test Categories
- **Unit Tests**: Cover filtering, search parsing, resource-set digests, manifest canonicalization, TOCTOU safety, TTY inputs, HMAC tokens, expiration, and replay prevention.
- **Contract Tests**: Validate assets, manifests, receipts, events, authorization claims, and backups against JSON schemas. Bridge tests simulate helper outputs, structured per-item import evidence, malformed JSON, protocol mismatches, timeouts, stderr noise, and non-zero exit codes.
...
An iteration is complete when Ruff, Python tests (119 total), Swift tests (11 total), package builds, and publication scans pass, and unexecuted live steps are recorded in [working.md](working.md).
